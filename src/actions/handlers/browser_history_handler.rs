...
impl BrowserHistoryHandler {
    ...
    /// Get cached browser history entries or refresh cache if needed
    pub fn get_history_entries() -> Vec<HistoryEntry> {
        let cache_mutex = HISTORY_CACHE.lock().unwrap();
        let last_update_mutex = LAST_CACHE_UPDATE.lock().unwrap();

        // Check if cache is still valid (less than 5 minutes old)
        if cache_mutex.is_some() && last_update_mutex.elapsed().unwrap_or(Duration::from_secs(600)) < Duration::from_secs(300) {
    ...
    fn refresh_history_cache() -> Vec<HistoryEntry> {
        ...
        info!("Found {} unique browser history entries across all browsers", unique_entries.len());
        unique_entries
    }
...
impl ActionDefinition for BrowserHistoryHandler {
    fn create_action(&self, db: Arc<Database>, cx: &mut Context<ActionListView>) -> ActionItem {
        ...
        ActionItem::new(
            self.get_id(),
            self.get_name(),
            vec!["history".to_string(), "browser".to_string()],
            "Open browser history URL".to_string(),
            self.clone(),
            |_input: &str| false, // Main handler is never shown, only specific entries
            move || {
                div()
                    .flex()
                    .gap_4()
...
impl HistoryCollector {
    ...
    fn collect_all_browser_histories() -> Vec<HistoryEntry> {
        ...
        for (browser_type, browser_paths) in browsers {
            if let Ok(browser_entries) = Self::get_browser_history(browser_type, &browser_paths) {
                info!(
    ...
    fn get_browser_history(browser_type: BrowserType, db_paths: &[PathBuf]) -> Result<Vec<HistoryEntry>> {
        match browser_type {
            BrowserType::Firefox => Self::get_firefox_history(db_paths),
            _ => Self::get_chromium_based_history(browser_type, db_paths),
        }
    }
    
    /// Get Firefox history from all possible profile directories
    fn get_firefox_history(firefox_dirs: &[PathBuf]) -> Result<Vec<HistoryEntry>> {
        let mut entries = Vec::new();
            ...
            for dir_entry in fs::read_dir(firefox_dir)? {
                ...
                if path.is_dir() {
                    ...
                    if places_db.exists() {
                        ...
                        info!("Successfully copied Firefox database to temporary location: {:?}", temp_db);
                        
                        if let Ok(profile_entries) = SqliteHistory::read_firefox_db(&temp_db) {
                            info!("Successfully read {} entries from Firefox profile: {:?}", profile_entries.len(), path);
    ...
    fn get_chromium_based_history(browser_type: BrowserType, db_paths: &[PathBuf]) -> Result<Vec<HistoryEntry>> {
        let mut entries = Vec::new();
        ...
        for db_path in db_paths {
            ...
            debug!("Found {} history database: {:?}", Self::browser_type_to_string(browser_type), db_path);
            ...
            if let Ok(browser_entries) = SqliteHistory::read_chromium_db(&temp_db) {
                entries.extend(browser_entries);
            }
    ...
    fn build_browser_paths(home_dir: &str, browser_type: BrowserType, install_types: &[InstallType]) -> Vec<PathBuf> {
        // Firefox is special because we need to search directories for profiles
        if browser_type == BrowserType::Firefox {
            let firefox_paths: Vec<PathBuf> = install_types.iter().map(|&install_type| {
                match install_type {
                    InstallType::Standard => Path::new(home_dir).join(".mozilla/firefox"),
                    InstallType::Snap => Path::new(home_dir).join("snap/firefox/common/.mozilla/firefox"),
                    InstallType::Flatpak => Path::new(home_dir).join(".var/app/org.mozilla.firefox/.mozilla/firefox"),
                }
            }).collect();
        ...
        let base_paths = match browser_type {
            ...
            BrowserType::OperaDeveloper => vec![
                ".config/opera-developer/History"
            ],
        ...
        if browser_type != BrowserType::Firefox {
            debug!("{} browser paths to check: {:?}", Self::browser_type_to_string(browser_type), paths);
        }
...
// ============================================================================
// SQLite History - Handles direct database operations
// ============================================================================
...
impl SqliteHistory {
    ...
    fn firefox_history_query() -> &'static str {
    ...
    fn chromium_history_query() -> &'static str {
    ...
    fn read_firefox_db(db_path: &Path) -> Result<Vec<HistoryEntry>> {
        let conn = Self::open_connection(db_path)?;
        let mut entries = Vec::new();
        
        let mut stmt = conn.prepare(Self::firefox_history_query())?;
    ...
    fn read_chromium_db(db_path: &Path) -> Result<Vec<HistoryEntry>> {
        let conn = Self::open_connection(db_path)?;
        let mut entries = Vec::new();
        
        let mut stmt = match conn.prepare(Self::chromium_history_query()) {
            Ok(stmt) => stmt,
            Err(e) => {
...
// ============================================================================
// Browser History Factory - Creates action items from history entries
// ============================================================================
...
impl BrowserHistoryFactory {
    ...
    pub fn create_actions_for_query(query: &str, db: Arc<Database>, cx: &mut Context<ActionListView>) -> Vec<ActionItem> {
        if query.trim().is_empty() {
            return Vec::new();
        }
        
        let query_lower = query.to_lowercase();
        let entries = BrowserHistoryHandler::get_history_entries();
        info!("Searching {} browser history entries for '{}'", entries.len(), query);
        
        let config = cx.global::<Config>();
        let text_secondary_color = config.text_secondary_color;
        
        // Since SQL is now handling most of the filtering, just match query against title/URL
        let matching_entries: Vec<_> = entries.into_iter()
            .filter(|entry| {
        ...
        info!("Found {} matching browser history entries", matching_entries.len());
        
        matching_entries.into_iter().map(|entry| Self::create_action_from_entry(entry, db.clone(), &config)).collect()
    }
    
    /// Create an action item from a history entry
    fn create_action_from_entry(entry: HistoryEntry, db: Arc<Database>, config: &Config) -> ActionItem {
        let handler = BrowserHistoryHandler::with_entry(entry.clone());
        let display_title = if entry.title.is_empty() { entry.url.clone() } else { entry.title.clone() };
        let display_url = entry.url.clone();
        let name = display_title.clone();
        let text_secondary_color = config.text_secondary_color;
        
        // Create a static string ID that lives for the entire program
        let id_str = Box::leak(format!("browser-history-{}", 
            entry.url.chars().take(20).collect::<String>()).into_boxed_str());
        
        // Clone the display_url for use in closures
        let display_url_for_filter = display_url.clone();
        let display_title_for_filter = display_title.clone();
        
        ActionItem::new(
            ActionId::Builtin(id_str),
            name.clone(),
            vec!["history".to_string(), "browser".to_string()],
            format!("Open URL: {}", entry.url),
            handler,
            move |input: &str| {
                let input_lower = input.to_lowercase();
                display_title_for_filter.to_lowercase().contains(&input_lower) || 
                display_url_for_filter.to_lowercase().contains(&input_lower)
            },
            move || {
                div()
                    .flex()
            ...
            50 + entry.visit_count.min(100) as usize, // Higher relevance for more visited sites
            10,
            db,
        )
    }
